...
TaskThree::TaskThree(WindowContext& context, PerspectiveCamera& camera, RenderSettings& settings) 
    : SceneBase(context, camera, settings)
{
    ...
    pieceShader = ShaderComponent(m_multipleLightsVert, m_multipleLightsFrag);
    ...
    {
        "src/resources/textures/skybox/right.png", "src/resources/textures/skybox/left.png",
        "src/resources/textures/skybox/top.png", "src/resources/textures/skybox/bottom.png",
        "src/resources/textures/skybox/front.png", "src/resources/textures/skybox/back.png",
    };

    skyBox = new Cubemap(faces, ShaderComponent(skyBoxVertSrc, skyBoxFragSrc));

    setup_lights(LightingData);
}

void TaskThree::OnSetup()
{
...
void TaskThree::OnExit()
{
    //SaveCamSnapshot();
}

void TaskThree::OnUpdate(float deltaTime)
{
    ...
    // rotate lights around - more exciting
    ...
    const auto pieceColor = glm::vec3(1.0f);
    if (m_chessAnimShowPieces)
    {
        for (size_t i = 0; i < m_chessPieces.size(); i++)
        {
            //glm::vec3 cellColor = (i + j) % 2 == 0 ? glm::vec3(1.0f) : glm::vec3(0.0f);
            m_chessPieces[i]->Draw(pieceShader, View, Projection, pieceColor, Position,
                Front, LightingData.Directional, LightingData.PointLights, LightingData.Spot);
        }
    }


    // Draw skybox last
    skyBox->Draw(m_camera.GetViewMatrix(), m_camera.GetProjectionMatrix());
}

void TaskThree::OnGui()
{
	render_ui();
	handle_input();
}

void TaskThree::setup_lights(SceneLightingData& data)
{
...
std::vector<Drawable*> TaskThree::generate_chess_board(const char* cellTexturePath, const char* pieceTexturePath,
    const glm::vec3& darkPieceColor, const glm::vec3& lightPieceColor) const
{
    ...
    for (int i = 0; i < 8; i++)
    {
        for (int j = 0; j < 8; j++)
        {
            ...
            {
                // Dark
                const auto pawn = new Pawn(ShaderComponent(m_multipleLightsVert, m_multipleLightsFrag), darkPieceColor, pieceTexture);
...
void TaskThree::check_new_cam_pos() {
...
void TaskThree::play_piece_anim()
{
