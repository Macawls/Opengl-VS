...
public:
    ...
    Cylinder(const Shader& shader, const glm::vec3& color);
    Cylinder(const Shader& shader, const glm::vec3& color, const Texture& texture);
};

inline Cylinder::Cylinder(const Shader& shader, const glm::vec3& color): Drawable(shader, color)
{
...
inline Cylinder::Cylinder(const Shader& shader, const glm::vec3& color, const Texture& texture) : Drawable(shader, color, texture)
{
    Transform.GuiDisplay = "Cylinder";
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        for (int j = 0, k = 0; j <= sectors; ++j, k += 3)
        {
            ...
            float t = 1.0f - static_cast<float>(i) / stacks;
    ...
    {
        float h = -height / 2.0f + i * height;              // z value  [-h/2 to h/2]
        float nz = -1 + i * 2;                              // z value normal [-1 to 1]
        ...
        for (int j = 0, k = 0; j < sectors; ++j, k += 3)
        {
            ...
            // Position
            m_vertices.push_back(ux * rad);
            m_vertices.push_back(uy * rad);
            m_vertices.push_back(h);
